...
static int	handle_special_chars(char *string, int *i, t_env **env, int flag)
{
	...
	if (is_single_quote(c) && flag != 2)
		process_single_quote(string, i, &len);
...
static int	process_regular_char(char *string, int *i, int flag)
{
	if (string[*i] && string[*i] == ' ' && flag != 2)
	{
		(*i)++;
		return (0);
	}
	else if (string[*i] && !is_double_quote(string[*i])
		&& !is_single_quote(string[*i]) && flag != 2
		&& string[*i] != '$')
	{
		(*i)++;
		return (1);
	}
	else if (string[*i] && flag == 2)
	{
		(*i)++;
		return (1);
	}
...
int	count_new_args_length(char *string, t_env **env, int flag)
{
	...
	while (string[i])
	{
		...
		if (special_len > 0)
			...
			total_len += process_regular_char(string, &i, flag);
	}
	return (total_len);
}
